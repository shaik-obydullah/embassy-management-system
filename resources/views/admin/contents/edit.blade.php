...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Content Page</h1>
    ...
    <form method="POST" action="{{ route('admin.contents.update', $content) }}">
        ...
        <div class="mb-4">
            ...
            <textarea name="body" id="body" rows="12"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 @error('body') border-red-500 @enderror">{{ old('body', $content->body) }}</textarea>
            @error('body')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="meta_description" id="meta_description" rows="2"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 @error('meta_description') border-red-500 @enderror">{{ old('meta_description', $content->meta_description) }}</textarea>
